...
abstract class OHsm {
	...
	def InitHsm() = {
		...
		assert (GetSuperState(myState) == state, {println("initial transition must go *one* level deep.")})
		...
		Trigger(state, Entry());
		while(Trigger(state, Init()) == null) // init handled?
		{
			assert (GetSuperState(myState) == state, {println("mistmatched super state.")})
			state = myState;
	...
	/// </summary>
	private def InitializeState(state : QState) {
		myState = state;
	}
